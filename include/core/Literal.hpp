...
public:
    ...
    virtual void set(ValueVariant) { throw std::runtime_error("[ERROR] Не поддерживается функция установки значения"); }
    virtual std::shared_ptr<Literal> getIdx(const Literal&) const { throw std::runtime_error("[ERROR] Не поддерживается функция получения индекса"); }
    virtual std::shared_ptr<Literal> getIdxDirect(int) const { throw std::runtime_error("[ERROR] Не поддерживается функция получения индекса напрямую"); }

    // Арифметика
    virtual std::shared_ptr<Literal> add(const Literal&) const { throw std::runtime_error("[ERROR] + не поддерживается"); }
    virtual std::shared_ptr<Literal> sub(const Literal&) const { throw std::runtime_error("[ERROR] - не поддерживается"); }
    virtual std::shared_ptr<Literal> mul(const Literal&) const { throw std::runtime_error("[ERROR] * не поддерживается"); }
    virtual std::shared_ptr<Literal> div(const Literal&) const { throw std::runtime_error("[ERROR] / не поддерживается"); }
    virtual std::shared_ptr<Literal> unaryMinus() const { throw std::runtime_error("[ERROR] \'-\' не поддерживается"); }

    // Логика
    virtual std::shared_ptr<Literal> logical_or(const Literal&) const { throw std::runtime_error("[ERROR] || не поддерживается"); }
    virtual std::shared_ptr<Literal> logical_and(const Literal&) const { throw std::runtime_error("[ERROR] && не поддерживается"); }
    virtual std::shared_ptr<Literal> logical_not() const { throw std::runtime_error("[ERROR] \'!\' не поддерживается"); }

    // Сравнения
    virtual std::shared_ptr<Literal> equal(const Literal&) const { throw std::runtime_error("[ERROR] == не поддерживается"); }
    virtual std::shared_ptr<Literal> not_equal(const Literal&) const { throw std::runtime_error("[ERROR] != не поддерживается"); }
    virtual std::shared_ptr<Literal> less(const Literal&) const { throw std::runtime_error("[ERROR] < не поддерживается"); }
    virtual std::shared_ptr<Literal> less_equal(const Literal&) const { throw std::runtime_error("[ERROR] <= не поддерживается"); }
    virtual std::shared_ptr<Literal> greater(const Literal&) const { throw std::runtime_error("[ERROR] > не поддерживается"); }
    virtual std::shared_ptr<Literal> greater_equal(const Literal&) const { throw std::runtime_error("[ERROR] >= не поддерживается"); }

    // Печать
    virtual void printLex(std::ostream &os) const = 0;
    virtual void print(std::ostream &os) const = 0;
    virtual void scan(std::istream &is) { throw std::runtime_error("[ERROR] Ввод не поддерживается"); }
};
...
public:
    ...
    void print(std::ostream &os) const override { throw std::runtime_error("[ERROR] Вывод не поддерживается"); }
};
...
public:
    ...
    ValueVariant evaluate() const override { return value_; }
    std::any get() const override { return value_; }
    std::shared_ptr<Literal> logical_and(const Literal& other) const { return std::make_shared<Bool>(value_ && std::get<bool>(other.evaluate())); }
...
public:
    ...
    void set(ValueVariant v) override {
        if (!std::holds_alternative<double>(v)) throw std::runtime_error("[ERROR] Несовпадение типов");
    ...
    void printLex(std::ostream &os) const override { os << "ConstantReal(" << value_ << ")"; }
...
public:
    ...
    std::any get() const override { return value_; }
    void set(ValueVariant v) override {
        if (!std::holds_alternative<int>(v)) throw std::runtime_error("[ERROR] Несовпадение типов");
    ...
    void printLex(std::ostream &os) const override { os << "ConstantInt(" << value_ << ")"; }
    void print(std::ostream &os) const override { os << value_ << "\n"; }
    void scan(std::istream &is) override { is >> value_; }
};
...
public:
    ...
    void printLex(std::ostream &os) const override { os << "IndirectIdxReal(" << reinterpret_cast<void*>(value_) << ")"; }
...
public:
    ...
    void printLex(std::ostream &os) const override { os << "IndirectIdxInt(" << reinterpret_cast<void*>(value_) << ")"; }
...
public:
    ...
    std::shared_ptr<Literal> getIdx(const Literal& other) const override { 
        ...
        else throw std::runtime_error("[ERROR] Индекс обязан быть целочисленным");
        if (idx < 0 || idx >= sizeT) throw std::out_of_range("[ERROR] Индекс выходит за границы массива");
    ...
    void printLex(std::ostream &os) const override { os << "LinkReal(" << name_ << ")"; }
...
public:
    ...
    void printLex(std::ostream &os) const override { os << "LinkInt(" << name_ << ")"; }
    void print(std::ostream &os) const override {
        auto it = VarMap.find(name_);
        int* ptr = reinterpret_cast<int*>(it->second.addr);
        os << *ptr << "\n";
